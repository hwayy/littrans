...
def plan_structure(
    ...
    for b in blocks:
        ...
        for line in b["lines"]:
            ...
            if current and indent and last_y is not None and y - last_y > font_size * 0.7:
...
def assemble_structure(units: list[SourceUnit], assets: dict[str, FidelityAsset], plan: dict[str, Any], make_unit: Callable[..., SourceUnit]) -> list[SourceUnit]:
    ...
    for u in units:
        ...
        previous_display = previous and any(
            assets[aid].display for aid in asset_reference_ids(previous.source_text)
        )
        if (
            previous
            ...
            and previous.kind.value != "heading"
            and u.kind.value != "heading"
