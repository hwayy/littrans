...
Keep target text semantic: the renderer owns heading/list/footnote wrappers. Literal markup such as `<Button>` remains inert text. Keep reader notes, terminology proposals and uncertainties outside the translation. An uncertain mathematical interpretation must be recorded and resolved before approval; an unfinished structured transcription alone is not such an uncertainty.
