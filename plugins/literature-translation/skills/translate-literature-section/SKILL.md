...
1. Read `manifest.yaml`, `source.md`, `context.md`, and `output-schema.json` completely. On revision, also read the current `translation.jsonl` and all review issues for the batch.
2. Translate only units listed in `translatable_unit_ids`. Preserve every `unit_id` and `source_hash`. Do not create translations for display formulas, code, or figures. Preserve verified inline `$...$` LaTeX byte-for-byte inside the translated sentence.
...
6. Run `translation submit`, then `qa run`. Fix every deterministic error and rerun both commands. Do not weaken protected-token or numeric checks to make a batch pass.
