...
1. Read the translate packet's source, context, output schema, glossary and `original-images.json`. Inspect the original page and asset PNGs needed to understand every source unit. Codex writers use a fresh `gpt-5.6-luna` task at `max` effort. Do not wait for or default to reading transcription candidates.
