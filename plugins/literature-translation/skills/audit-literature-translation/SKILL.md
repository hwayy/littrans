...
3. Original-image formulas are valid reading content. Do not report unfinished LaTeX as a translation defect. If the original mathematical meaning is unclear or translated incorrectly, report it; separate an image coverage defect from a candidate-transcription defect.
...
5. Return precise JSONL issues using the issue contract. Import each performed lens, including an empty result, with `review import` or the coordinated `review import-set`. Never claim an independent lens that was not actually performed.
