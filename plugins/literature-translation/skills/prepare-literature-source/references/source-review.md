...
4. Create a fresh packet for the pages that still need review and repeat. After three rounds without approval, stop and report the page as blocked, with the reason.
...
- **Profile rules** are hypotheses to test against the original. If you meet a form the profile does not cover, do not approve the pages that print it; propose a `page_rules` block for them in your report. A packet without `document_structure` has no profile: judge each form from the page itself and mention the missing profile in your report.
