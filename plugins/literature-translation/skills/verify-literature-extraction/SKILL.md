...
5. Restore code indentation exactly, set `code_language`, and confirm that literal tags such as `<Button>` are inside code or remain literal prose. Mark Note/Tip/Warning blocks as `note`. For a titled multi-paragraph sidebar, preserve the original heading/paragraph kinds and assign one shared `sidebar_id`, with `sidebar_role: title` on its heading and `sidebar_role: body` on every contained unit.
...
- Never mark a formula, table, figure, or code block verified without comparing it to the rendered PDF page.
