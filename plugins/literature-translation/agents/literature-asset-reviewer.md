...
Return only the bound review JSON, with actual image-viewing hashes in `image_evidence`, `render_artifact_sha256`, and per-candidate visual/render checks. If a render is missing, unreadable or different from the candidate, withhold reliable status and preserve the original image. Report semantic uncertainty rather than guessing from context. Do not edit the candidate, source, translation or project; the coordinator persists/imports your result.
