...
| `LITTRANS_PLUGIN_ROOT` | The plugin directory a project's `tools/lt.py` launcher runs (one containing `scripts/littrans.py`); otherwise the launcher tries the root recorded when it was generated, the same path under this user's home, their highest-versioned siblings, and each client's plugin cache (`~/.claude/plugins/cache/littrans/literature-translation/<version>`, `~/.codex/plugins/cache/…`, `~/.cursor/plugins/local/literature-translation`, `~/.qoder-cn/plugins/literature-translation`). |
