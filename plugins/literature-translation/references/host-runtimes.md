...
| Host | Batch wave default | Batch wave maximum | Per-dispatch model | Per-dispatch effort | Role policy |
| --- | ---: | ---: | :-: | :-: | --- |
| Codex | 3 | 3 | yes | yes | `agent_models.codex` (recommended: fresh `gpt-5.6-luna`, `max` effort) |
| Cursor | 6 | 9 | no | no | `agent_models.cursor`: the host's own policy decides |
| Claude Code | 3 | 6 | yes | no | `agent_models.claude` (recommended: `sonnet`; effort comes from the writer agents' frontmatter) |
| Qoder | 3 | 6 | no | no | `agent_models.qoder`: the host's own policy decides |
...
Role models are not hard-coded. The plugin ships recommended defaults per host in `profiles/host-models.yaml`; `project init` copies them into the project's `project.yaml` under `agent_models`, where each project should confirm or override them. Four roles configure a dispatch independently of one another — `translate` (which also covers `revise`), `transcribe`, `audit` and `asset-audit` — and each carries **its own** `model` and `reasoning_effort`; no effort is shared between roles. Every configured model is a **dispatch value**: what the coordinator hands to the host's task launcher, a category alias (`sonnet` on Claude Code) or a concrete id as the host's convention has it. Which model the host serves under that value is the host's own configuration; the plugin never verifies a dispatched subagent's model.

Leaving a role, a model or an effort unset is a supported choice, not an error: that dispatch then follows the host's own default subagent behaviour, which is the only possibility on Cursor and Qoder and a legitimate preference on Codex and Claude Code. The CLI reports the two mismatches — a value configured for a host whose launcher cannot take it, and an unset value on a host whose launcher can — as advisories on stderr, in the `dispatch_advisories` field of `workflow next` and `workflow status`, and in `project models PROJECT --host HOST`. An advisory never blocks a packet, and a configured value the host cannot take is still recorded in the packet rather than dropped.
...
The coordinating session runs the CLI itself and dispatches one fresh subagent per packet. Pass the packet path, the project path and, when the packet records one, its `model` (the Agent tool's `model` parameter, e.g. `sonnet`); a packet with no model dispatches on the session's default. The Agent tool has no effort parameter, so a configured `reasoning_effort` is advisory here: the writer agents declare `effort: high` in their frontmatter, and the CLI says so rather than blocking. The alias is resolved by the host's provider configuration and may land on any model; a transcriber echoes `sonnet` in its submission and, if it knows what was served, records that in `served_model_label`. Reviewer agents are restricted to `Read`, `Glob` and `Grep`, so they cannot write project files: the parent saves and imports their JSONL issues or review JSON. Translators and transcribers run `translation submit`, `qa run` or `assets submit` themselves and report the outcome. Independence rules are unchanged: give a subagent only its packet, never another worker's candidate or an expected verdict.
...
The coordinating session runs the CLI itself and dispatches one fresh subagent per packet. Pass the packet path and the project path. Qoder does not let a coordinator choose the model of a single dispatched task, so `agent_models.qoder` ships empty and packets record no model: the host's own policy decides. Configuring a value anyway is allowed — the packet records it and `project models PROJECT --host qoder` reports that the plugin cannot pass it — and nothing is blocked either way. Reviewer agents are restricted to `Read`, `Glob` and `Grep`, so they cannot write project files: the parent saves and imports their JSONL issues or review JSON. Translators and transcribers run `translation submit`, `qa run` or `assets submit` themselves and report the outcome. Independence rules are unchanged: give a subagent only its packet, never another worker's candidate or an expected verdict.
